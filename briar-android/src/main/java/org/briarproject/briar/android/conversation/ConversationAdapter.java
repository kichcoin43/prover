...
class ConversationAdapter
                ...
                org.briarproject.briar.android.emoji.MessageReactionManager.ReactionListener {
        ...
        ConversationAdapter(Context ctx,
                ...
                // Register for reaction updates
                reactionManager.addListener(this);
        }
        
        @Override
        public void onReactionAdded(org.briarproject.briar.android.emoji.MessageReaction reaction) {
                // Find and refresh the item with this message ID
                String messageId = reaction.getMessageId();
                for (int i = 0; i < items.size(); i++) {
                        if (items.get(i).getKey().equals(messageId)) {
                                notifyItemChanged(i);
                                break;
                        }
                }
        ...
        @Override
        public void onReactionRemoved(org.briarproject.briar.android.emoji.MessageReaction reaction) {
                // Find and refresh the item with this message ID
                String messageId = reaction.getMessageId();
                for (int i = 0; i < items.size(); i++) {
                        if (items.get(i).getKey().equals(messageId)) {
        ...
        public ConversationItemViewHolder onCreateViewHolder(ViewGroup viewGroup,
                ...
                if (type == R.layout.list_item_conversation_msg_in) {
                        return new ConversationMessageViewHolder(v, listener, true,
                                        imageViewPool, imageItemDecoration, reactionManager);
